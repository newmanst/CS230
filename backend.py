...
class TinyYoloFeature(BaseFeatureExtractor):
	...
	momentum = 0.99
        ...
        x = BatchNormalization(name='norm_1', momentum = momentum)(x)
        x = LeakyReLU(alpha=0.1)(x)
        x = MaxPooling2D(pool_size=(2, 2))(x)
        ...
        for i in range(0,4):
            ...
            x = BatchNormalization(name='norm_' + str(i+2), momentum = momentum)(x)
            x = LeakyReLU(alpha=0.1)(x)
            x = MaxPooling2D(pool_size=(2, 2))(x)

        # Layer 6
        x = Conv2D(512, (3,3), strides=(1,1), padding='same', name='conv_6', use_bias=False)(x)
        x = BatchNormalization(name='norm_6', momentum = momentum)(x)
        x = LeakyReLU(alpha=0.1)(x)
        ...
        for i in range(0,2):
            ...
            x = BatchNormalization(name='norm_' + str(i+7), momentum = momentum)(x)
            x = LeakyReLU(alpha=0.1)(x)
